'use strict';
// done in collaboration with Alan Hung and Seid Mohamed
...
const templateWithJQuery = () => {
  ...
  for(let i=0; i<starWarsPeople.length; i++ ){
    const myPeople = $('#template').clone();
   
    myPeople.find('h2').text(starWarsPeople[i].name);
    myPeople.find('h3').text(starWarsPeople[i].height);
    myPeople.find('p').text(starWarsPeople[i].eye_color);

    $('main').append(myPeople);
    myPeople.removeAttr('id');
...
for(let i=0; i<str.length; i++){
  emptyArray.push(str.slice(i,i+1));
}
return emptyArray;
};
...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here
  let niceFoods = recipe.ingredients;
  let foodsList;
  for(let i=0; i<2; i++){
    for(let j=0; j<niceFoods.length; j++){
      foodsList = niceFoods[j];
      niceFoods[j] = foodsList.slice(foodsList.indexOf(' ')+1, foodsList.length);
    }
  }
  result = niceFoods;
  return result;
...
//   for(let i=0; i<2; i++){
//     for(let j=0; j<niceFoods.length; j++){
//       foodsList = niceFoods[j] = foodsList.slice(foodsList.indexOf('')+1, foodsList.length);
// 
//     }
//   }
//   result = niceFoods;
